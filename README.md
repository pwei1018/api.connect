...
If no relevant context is found, it tags `@pwei1018` (configurable via the `QA_BOT_MAINTAINERS_MENTION` repository variable) for manual follow-up instead of guessing. It generates answers using the [GitHub Copilot SDK](https://docs.github.com/en/copilot/sdk) (`@github/copilot-sdk`), which drives the Copilot CLI runtime programmatically instead of shelling out to the `copilot` CLI binary — no tool-use permissions are granted, so answers are grounded only in the context spliced into the prompt. Authenticated via the built-in `GITHUB_TOKEN` with the `copilot-requests: write` permission — no external secrets required (assuming your org's "Allow use of Copilot CLI billed to the organization" policy is enabled; otherwise a PAT can be substituted, see the workflow's comments).
